...
vec4 grid(float modX, float modY, float strength) {
	float density = 0.4 * (1.0 - abs((2.0-min(modX, modY))/2.0)); // grid alpa 0.4 and then anti alias.
	return vec4(1.0-density*strength, 1.0-density*strength, 1.0-density*strength, 1.0);
}
...
void main() {
    ...
    for(int i = 0; i<=6; i++) {
        ...
        float fade = max(0.0, 0.9-distance/resLength);
	...
	gl_FragColor = grid(modX, modY, strength);// + vec4(0.1, 0.1, 0.1, 1.0);
}